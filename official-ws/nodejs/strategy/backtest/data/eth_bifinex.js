...
function getTradeHistory(params) {
  var path = '/candles/trade:5m:tETHUSD/hist'
...
async function requestData(params) {
  ...
  if (Array.isArray(dataList)) {
    // 源数据为[t, o, c, l, h, v] => [isoT, o, h, l, c, v]
    return dataList.map(item => [new Date(item[0]).toISOString(), item[1], item[3], item[4], item[2], item[5]])
  } else {
    throw dataList
  }
}
