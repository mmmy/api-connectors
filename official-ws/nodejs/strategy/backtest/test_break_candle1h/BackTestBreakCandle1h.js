...
class BackTestBreakCandle1h extends BackTest {
  constructor(options) {
    super(options)
  ...
  initStrategy() {
    this.setStrategy((bar, candles) => {
      ...
      const signal = _1hCandle.isLastBarTrend(len)
      ...
      if (
        ...
        const upVolFilter = upVol ? _1dCandle.isUpVol(10, 3) : true
        const highVolFilter = highVol ? _1hCandle.isHighVol(len, 3) : true
  ...
  readBar1h(bar) {
    ...
    if (hasStopOpenOrder) {
      ...
      const stochOverSignal = this.getCandleByPeriod('1h').stochOverTradeSignal(9, 3, 30, 70)
  ...
  setPositionStop(long) {
    ...
    const { maxHigh, minLow } = this.getCandleByPeriod('1h').getMinMaxHighLow(len)
    this._accout.setStopPrice(long ? minLow : maxHigh)
  }

  setPositionProfit(long) {
    const { len } = this._options
    const { high, low } = this.getHistoryCandleByPeriod('1h', 2)
    const { maxHigh, minLow } = this.getCandleByPeriod('1h').getMinMaxHighLow(len)
...
module.exports = BackTestBreakCandle1h